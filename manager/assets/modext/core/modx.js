...
Ext.extend(MODx, Ext.Component, {
    ...
    add: function(cmp) {
        ...
        const ctr = Ext.getCmp('modx-content');
        if (ctr) {
            ctr.removeAll();
            ctr.add(cmp);
            ctr.doLayout();
    ...
    onAjaxException: function(conn, response, opt, event) {
        try {
            response = Ext.decode(response.responseText);
        } catch (e) {
            const matched = response.responseText.match(/<body[^>]*>([\w|\W]*)<\/body>/im);
            let text;
            if (typeof (matched[1] !== 'undefined')) {
                text = `<p>${e.message}:</p>${matched[1]}`;
            } else {
                text = `${e.message}: ${response.responseText}`;
            }
            Ext.MessageBox.show({
                title: _('error'),
                msg: text,
        ...
        if (response && (response.code === 401 || (response.object && response.object.code === 401))) {
            if (!MODx.loginWindow) {
                MODx.loginWindow = MODx.load({
                    xtype: 'modx-window-login',
                    username: MODx.useresponse.username
                });
            }
            ...
            listeners: {
                success: {
                    fn: function() {
                        ...
                        const cmp = Ext.getCmp('modx-panel-resource');
    ...
    getPageStructure: function(items, config = {}) {
        Ext.applyIf(config, {
            xtype: 'modx-tabs',
            itemId: 'tabs',
            items: items,
    ...
    hideTab: function(containerId, regionId) {
        this.hideRegion(containerId, regionId);
    },
    ...
    _getNextActiveTab: function(tabPanel, tabId) {
        let id = tabId;
        if (MODx.hiddenTabs.indexOf(tabId) !== -1) {
            for (let i = 0; i < tabPanel.items.items.length; i++) {
                id = tabPanel.items.items[i].id;
    ...
    createResource: function(record) {
        ...
        MODx.createResourceWindow = MODx.load({
            ...
            listeners: {
                success: {
                    fn: function(response) {
                        MODx.loadPage(`?a=resource/update&id=${response.a.result.object.id}`);
                    },
                    scope: this
                },
                failure: {
    ...
    switchLanguage: function(lang) {
        ...
        Ext.iterate(MODx.request, (key, value) => {
            ...
            success: function(response, options) {
                ...
                if (responseData.success) {
                    if (config.listeners.success && config.listeners.success.fn) {
                        this._runCallback(config.listeners.success, [responseData]);
                    }
                } else if (config.listeners.failure && config.listeners.failure.fn) {
                    this._runCallback(config.listeners.failure, [responseData]);
                    MODx.form.Handler.errorJSON(responseData);
                }
                return true;
            },
...
Ext.extend(MODx.form.Handler, Ext.Component, {
    ...
    errorExt: function(response, frm) {
        this.unhighlightFields();
        if (response && response.errors !== null && frm) {
            frm.markInvalid(response.errors);
        }
        if (response && response.message !== undefined && response.message !== '') {
...
Ext.extend(MODx.Msg, Ext.Component, {
    ...
    status: function(opt) {
        ...
        const markup = this.getStatusMarkup(opt),
              m = Ext.DomHelper.overwrite(MODx.stMsgCt, { html: markup }, true),

              fadeOpts = { remove: true, useDisplay: true };
        if (!opt.dontHide) {
            if (!Ext.isIE8) {
...
Ext.extend(MODx.HttpProvider, Ext.state.Provider, {
    initState: function(state) {
        if (state instanceof Object) {
            Ext.iterate(state, function(name, value) {
    ...
    onWriteSuccess: function(response, options) {
        const responseData = Ext.decode(response.responseText);
        if (responseData.success !== true) {
            this.dirty = true;
        } else {
            Ext.iterate(options.queue, function(name, value) {
                if (!name) {
                    return;
                }
    ...
    onWriteFailure: function(response) {
        this.dirty = true;
    ...
    onReadSuccess: function(response) {
        ...
        if (responseData.success === true && responseData.message) {
            state = Ext.decode(responseData.message);
            if (!(state instanceof Object)) {
                return;
            }
            Ext.iterate(state, function(name, value) {
