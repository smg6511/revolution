...
MODx.util.insertTagCopyUtility = function(cmp, elType) {
    ...
    ;
    /*
        If the helptag isn't available, skip here. This may happen when a lexicon is
        missing or outdated and doesn't contain the `example-replace-name` class.
    */
    if (!helpTag) {
        return;
    }
...
MODx.util.Format = {
    ...
    staticElementPathFragment: function(fragment, isDirectoryFragment = false) {
        fragment = fragment.replace(/[^\w\s-]/gi, '');
